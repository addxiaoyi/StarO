...
function getPool() {
    if (process.env.DATABASE_URL) {
        if (!pgPool) {
            pgPool = new pg_1.Pool(__assign(__assign({ connectionString: process.env.DATABASE_URL }, db_config_1.dbPoolConfig), { client_encoding: "utf8", application_name: process.env.OTEL_SERVICE_NAME || "starx-oauth" }));
